def convert_RealImag_to_MagPhs(folder_path):
    '''
        Convert Real/Imag in FFS format to Mag/Phase for theta and phi components of E-fields
        Parameters:
        ----------
        folder_path: Str
            The folder path of a directory including "a number" folders
    '''
    parameter_dict = get_paramters(folder_path)
    a_numbers = list(parameter_dict.keys())
    for a_number in a_numbers:
        outdir = os.path.join(folder_path, a_number+'/txt')
        if os.path.exists(outdir) is not True:
            os.mkdir(outdir)
        folder_path_a_number = glob.glob(os.path.join(folder_path, a_number+'/ffs'))[0]
        filenames = sorted(glob.glob(os.path.join(folder_path_a_number,'*ffs')))
